function [thresh,cntR,sumR,cntP,sumP] = boundaryPR_NO_Morph(pb,segs,nthresh)
...
%end
  for i = 1:nsegs,
   bmaps{i} =double(segs{i});
   if (max(bmaps{i}(:))>1) bmaps{i}=bmaps{i}./255;end
  end
...
for t = 1:nthresh,    
  ...
  for i = 1:nsegs,
    ...
    [match1,match2] = correspondPixels(bmap,bmaps{i},0.0078);
